...
def upgrade():
    ...
    direction = sa.Enum(*direction_options, name='direction')
    delay_accuracy_options = ('low', 'accurate, high')
    delay_accuracy = sa.Enum(*delay_accuracy_options, name='delayaccuracy')
    ...
    sa.Column('id', sa.VARCHAR(length=32), autoincrement=True, nullable=False),
    sa.Column('name', sa.VARCHAR(length=32), autoincrement=False, nullable=False),
    ...
    sa.ForeignKeyConstraint(['alert_id'], ['alerts.alert_id'], name='alert_effect_period_alert_id_fkey'),
    ...
    sa.ForeignKeyConstraint(['routes'], ['routes.id'], name='alert_affected_services_route_id_fkey'),
    sa.ForeignKeyConstraint(['alert_id'], ['alerts.alert_id'], name='alert_affected_services_alert_id_fkey'),
    sa.PrimaryKeyConstraint('id', name='alert_affected_services_pkey')
    )
    op.create_table('alert_events',
    sa.Column('id', sa.INTEGER(), autoincrement=True, nullable=False),
    sa.Column('trip_id', sa.VARCHAR(length=100), autoincrement=False, nullable=True),
    ...
    sa.Column('direction', sa.Enum(direction), autoincrement=False, nullable=True),
    ...
    sa.Column('delay_accuracy', sa.Enum(delay_accuracy), autoincrement=False, nullable=True),
    sa.CheckConstraint('day >= 0 AND day <= 6', name='alert_events_day_check')),
    sa.ForeignKeyConstraint(['routes'], ['routes.id'], name='alert_events_route_fkey'),
...
def downgrade():
    ...
    op.drop_constraint('alert_events_route_fkey', 'alert_events', type_='foreignkey')
